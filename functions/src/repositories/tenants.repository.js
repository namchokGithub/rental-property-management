...
async function findAllByProperty(propertyId, filters = {}) { let query = collection.where("propertyId", "==", propertyId); if (filters.status) query = query.where("status", "==", filters.status); return (await query.orderBy("fullName", "asc").get()).docs.map(toDocument); }
