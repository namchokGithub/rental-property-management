...
const collection = db.collection("otherChargeMasters");

function toDocument(snapshot) {
  return snapshot.exists ? { id: snapshot.id, ...snapshot.data() } : null;
}

async function findAllByProperty(propertyId, { isActive } = {}) {
  let query = collection.where("propertyId", "==", propertyId);
  if (typeof isActive === "boolean") query = query.where("isActive", "==", isActive);
  const snapshots = await query.orderBy("nameTh", "asc").get();
  return snapshots.docs.map(toDocument);
}
