...
- **OtherChargeMaster** (`src/types/otherCharge.ts`) — `id, nameTh, nameEn?, defaultAmount, isActive, createdAt, updatedAt`. Reusable master data for *optional* per-bill charges (garbage, meter maintenance, parking, internet, cleaning, etc.). A user can also explicitly pick one from the Settings-managed list (the dropdown only offers masters not already on the bill), at which point its `defaultAmount` is copied into a new `BillingCharge` that can then be edited per bill without touching the master. See Business Rules below for when active masters are auto-added vs. left to manual pick.
