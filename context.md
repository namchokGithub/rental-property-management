...
- `exceljs` — parses uploaded `.xlsx` files and generates the downloadable template for the Rooms page's bulk import feature (`src/lib/excel.ts`); dynamically imported (`import("@/lib/excel")`) at the two call sites in `RoomImportDialog.tsx` so it isn't in the app's main JS bundle
...
src/data/repositories/
  ...
  propertyRepository.ts      # live property registry list for the active-property selector
...
Every list-table menu (Rooms, Tenants, Billing, Invoices, and Settings' Other Charge Master list) paginates client-side, on top of the same already-filtered array Search/status/month/year filtering already produces — there is no server-side/query-level pagination (Firestore `limit`/`startAfter`) anywhere, matching this app's existing "load the collection, work with it client-side" convention.

- `src/hooks/usePagination.ts` — `usePagination(items, initialPageSize = 10)` returns `{ page, setPage, pageSize, setPageSize, totalPages, totalItems, pageItems }`. `page` is always clamped into `[1, totalPages]` (`Math.min(Math.max(page, 1), totalPages)`) so a stale page number past the end of a shrunk list can never render a blank page — it just falls back to the last valid page. `setPageSize` also resets to page 1, since a given item's row on a re-sized page is otherwise unpredictable. Exports `PAGE_SIZE_OPTIONS = [10, 20, 50, 100] as const`, the fixed, shared set of choices — not configurable per page.
- `src/components/common/Pagination.tsx` — the shared control: a "rows per page" `Select` (10/20/50/100) on one side, a `{{from}}-{{to}} of {{total}}` range label plus prev/next icon buttons on the other. `variant="card"` (default) renders its own `bg-card rounded-xl border shadow-sm` surface for placing directly below a page's table; `variant="bare"` (used only by the Settings Other Charge list, which already lives inside its own `Card`) drops that surface for a plain `border-t` separator instead, avoiding a nested card-in-card look.
- Each page passes `pageItems` (not the full filtered array) to its table component, and calls `setPage(1)` from every search/filter `onChange` handler and "clear filters" action — so changing a filter always lands back on page 1 rather than clamping to whatever page number happened to be selected before. `usePagination` itself does not auto-reset on filter changes (a `useMemo`-derived filtered array's identity changes on every unrelated live Firestore update too, not just on an actual filter change, so an automatic reset-on-array-identity-change would fire far too often) — resetting is each page's explicit responsibility instead.
- Bulk-select in `BillingTable`'s checkboxes/"select all" is scoped to the current page only (`records` prop is already `pageItems` by the time it reaches the table) — standard pagination behavior, no "select all across pages" affordance exists.
- **Deliberately not paginated:** the same set of tables Search excludes above (Dashboard's recent-billing preview, `RoomDetailSheet`/`TenantDetailSheet`'s per-entity billing history) — none of them are a full list-menu page.
...
- **Room** (`src/types/room.ts`) — `id, roomNumber, floor?, type?, monthlyRent, status, description?, electricityRate, waterRate, deletedAt, createdAt, updatedAt`. `status: "available" | "occupied" | "maintenance" | "inactive"`. Never stores a tenant reference. `deletedAt: string | null` — soft-delete marker, see Business Rules. Firestore path: `properties/{propertyId}/rooms/{roomId}`.
- **Tenant** (`src/types/tenant.ts`) — `id, name, phone?, email?, identificationNumber?, address?, emergencyContactName?, emergencyContactPhone?, status, notes?, deletedAt, createdAt, updatedAt`. `status: "active" | "inactive"`. `name` is a single free-text field (not split first/last) — see Data Migration History. `deletedAt: string | null` — soft-delete marker, see Business Rules. Firestore path: `properties/{propertyId}/tenants/{tenantId}`.
...
- **PropertySummary** (`src/types/property.ts`) — `id, name`, the selector-facing view of `properties/{propertyId}`. The parent document stores the property registry (`name`, optional `address`/`phone`, timestamps); Settings mirrors its editable identity values there whenever an admin saves.
- **PropertySettings** (`src/types/settings.ts`) — single record: property name/address/phone plus the three true monthly defaults (`defaultElectricityRate`, `defaultWaterRate`, `defaultInvoiceNote`) plus `otherChargeMasters?: OtherChargeMaster[]`, used to prefill new rooms and billing records. Fixed/optional fee amounts (garbage, meter maintenance, etc.) are **not** stored as their own fields — see `OtherChargeMaster` above. Firestore path: `properties/{propertyId}/settings/general`.
...
- **Validation** (`src/lib/validation.ts`): room number required and unique; monthly rent / rates ≥ 0; tenant first/last name required; billing current meter ≥ previous meter (both utilities), rent ≥ 0.
- **Room number uniqueness:** `validateRoom(input, existingRoomNumbers?)` takes an optional `Set<string>` of other rooms' lowercased/trimmed `roomNumber`s; if the submitted number (case-insensitively) matches one, it's `validation.room.roomNumberDuplicate` instead of being silently accepted. `RoomFormDialog` builds that set from the already-loaded `rooms` list passed down from `RoomsPage` (excluding the room currently being edited by id), so no extra Firestore read is needed — the check is naturally scoped to the active property, since `useRooms()` only ever holds that property's rooms. The Excel bulk-import path (`roomImportValidation.ts`) does its own equivalent duplicate check inline (against both existing rooms and other rows in the same file) rather than calling `validateRoom` with this parameter — the two call sites intentionally don't share that plumbing, since import also needs to catch in-file duplicates that `validateRoom` has no concept of.
- **Delete behavior:** every delete (room/tenant/billing) goes through `ConfirmDialog` (a Radix `AlertDialog` wrapper), never `window.confirm`. Every mutation shows a `sonner` toast. Delete itself is soft — see below.
- **Soft delete (rooms/tenants/billing):** `delete()` on `roomRepository`/`tenantRepository`/`billingRepository` no longer calls Firestore's `deleteDoc` — it sets `deletedAt: serverTimestamp()` (plus `updatedAt`) on the existing document, so a mis-click is recoverable by hand later instead of destroying data outright. `assertNoActiveAssignment()` still runs first for rooms/tenants, unchanged. Filtering is client-side inside the repository's read path (`firestoreCrud.ts`'s `getAll`/`subscribe`, and `billingRepository.subscribe`) — never a Firestore `where("deletedAt", "==", null)` query, since `==` never matches a document where the field is absent entirely (every pre-existing document), and this app has no backend or migration scripts to backfill one. Because filtering happens once, at the repository layer, every consumer (list pages, Dashboard aggregates, the room-number-uniqueness check, the `/invoices/:id` print page) is correct with zero changes of its own — a soft-deleted room's `roomNumber` becomes reusable automatically. Two deliberate exceptions: `billingRepository`'s `nextInvoiceNumber()` still scans *every* bill for the month regardless of `deletedAt` (a deleted bill's invoice number must never be reused), and `create()`'s one-bill-per-room-per-month guard still treats a soft-deleted month-slot as occupied (no restore UI exists yet — recovering that slot is a manual Firebase Console edit). Firestore Security Rules were tightened alongside this, not just the client call: `rooms`/`tenants` grant `create, update` only (no `delete`), and `billing`'s `delete` permission was removed with `deletedAt` added to its `update` field allowlist — rules are this app's only real enforcement, so closing the raw-delete path there too matters as much as the repository change. No UI/wording change — confirm dialogs and toasts still say "delete"; this is invisible to the end user. No `deletedBy` audit field and no restore/trash UI in this pass (see [spec](docs/superpowers/specs/2026-08-13-soft-delete-design.md)).
...
| Rooms                         | Done   | CRUD, detail sheet with billing history, assign/end tenancy, client-side search, pagination (10/20/50/100, default 10), Excel bulk import (upload -> validated preview -> parallel create)                                                                                                                                          |
| Tenants                       | Done   | CRUD, detail sheet, assign/move room, client-side search, pagination (10/20/50/100, default 10)                                                                                                                                                                                                                                      |
| Assignments                   | Done   | Exclusive active assignment per room, move-room handling                                                                                                                                                                                                                                      |
| Billing                       | Done   | Auto-calculated usage/totals, other charges, desktop table + mobile cards, client-side search, payment-status filter (draft/issued/paid/overdue via `resolveBillingStatus`), billing-month filter, checkbox row selection + bulk "Issue Selected" (draft records only, scoped to current page), frozen Actions column, pagination (10/20/50/100, default 10) |
| Invoices                      | Done   | List, in-app preview dialog, standalone print page, auto invoice numbering, client-side search, billing-month filter, pagination (10/20/50/100, default 10)                                                                                                                                                                          |
| Print Invoice                 | Done   | `/invoices/:id` standalone route, `@media print` + `@page A4`, browser print/Save-as-PDF                                                                                                                                                                                                      |
| Settings                      | Done   | Property info + the 3 true billing defaults (electricity rate, water rate, invoice note), plus a separate Other Charge Master (optional per-bill charges, paginated 10/20/50/100 default 10); used by new rooms and billing — see Data Migration History                                                                         |
| Responsive                    | Done   | Verified at 375/768/1024px via headless Chrome screenshots across all 6 top-level pages and their primary dialogs (room/tenant/billing detail sheets and the Excel import preview were not re-verified in this pass); sidebar becomes a Sheet drawer under `md`; Billing, Invoices, Rooms, Tenants tables and the Other Charge Master list become cards under `md`; header title truncates instead of wrapping below `md`                                                                             |
...
The Firestore schema that replaced the domain keys above — `users/{uid}`, `properties/{propertyId}`, and its `{rooms,tenants,assignments,billing,settings/general}` descendants — is documented in [docs/firebase/data-model.md](docs/firebase/data-model.md) and in Firebase Migration below.
...
| `src/property/PropertyContext.tsx`                                                                   | `PropertyProvider`, `useProperty()`, and `useActivePropertyId()` — loads the property registry, owns the active in-memory selection, and gates the app until a property is selected |
...
| `src/data/repositories/{property,room,tenant,assignment,billing,settings,otherCharge}Repository.ts`    | Per-entity Firestore repositories — see Repository architecture above for which use the generic factory vs. hand-written transactions                                                |
...
| `src/lib/excel.ts`, `src/features/rooms/roomImportValidation.ts`, `src/features/rooms/RoomImportDialog.tsx` | Rooms bulk-import feature: `.xlsx` parsing/template generation via `exceljs`, row validation/dedup reusing `validateRoom`, and the upload -> preview -> result dialog UI              |
